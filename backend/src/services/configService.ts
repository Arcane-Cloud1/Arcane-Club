...
export const getModerationConfig = async (): Promise<ModerationConfig> => {
  const setting = await prisma.systemSetting.findUnique({
    where: { key: 'moderation_config' },
  });
  if (setting) {
    return { ...defaultModerationConfig, ...JSON.parse(setting.value) };
  }
...
export const updateModerationConfig = async (config: ModerationConfig) => {
  await prisma.systemSetting.upsert({
    where: { key: 'moderation_config' },
    update: { value: JSON.stringify(config) },
    create: { key: 'moderation_config', value: JSON.stringify(config) },
  });
};
